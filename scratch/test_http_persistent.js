...
async function testFastDownload() {
    ...
    const outFile = path.join(__dirname, 'fast_test.mp4');

    if (fs.existsSync(outFile)) fs.unlinkSync(outFile);

    console.log('Testing FFmpeg with -http_persistent 1...');
    ...
    console.log(`FAST download finished in ${durationSec.toFixed(2)} seconds. Size: ${(stats.size / 1024 / 1024).toFixed(2)} MB`);
}

testFastDownload().catch(console.error);
